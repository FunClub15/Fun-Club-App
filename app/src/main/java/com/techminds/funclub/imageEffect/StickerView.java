...
public abstract class StickerView extends FrameLayout {
    ...
    public StickerView(Context context, List<SelectedImage> selectedImageList) {
        super(context);
    ...
    private OnTouchListener mTouchListener = new OnTouchListener() {
        ...
        public boolean onTouch(View view, MotionEvent event) {
           index = 0;
            ...
            Log.e("object list size", selectedImageList.size() + "");
            Log.e("index", index + "");
            if (view.getTag().equals("DraggableViewGroup")) {
                ...
                switch (event.getAction()) {
                    ...
                    default:
                        ...
                        index = view.getId();
                        Log.e("index in default", index + "");

                }
            }
            selectedImageList.get(index).setPostitionX(view.getX() + "");
            selectedImageList.get(index).setPostitionY(view.getY() + "");
            selectedImageList.get(index).setWidth(view.getWidth() + "");
            selectedImageList.get(index).setHight(view.getHeight() + "");

            if (angle > 0)
                selectedImageList.get(index).setAngel(angle + "");

            Gson gson = new Gson();

            String obj = gson.toJson(selectedImageList);
            Log.e("Rotation ->>", angle + "");
            UserPrefs.setSelectedImage(obj, context);
            return true;
        }
    };
